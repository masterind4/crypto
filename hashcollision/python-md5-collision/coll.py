...
if not os.path.exists(FASTCOLL_PLACE):
    ...
    try:
        ...
        raise Exception('Could not get the fastcoll source code. The download link may be broken.\nURL: {}\nError: {}'.format(FASTCOLL_LOC, e))
    except urllib.error.URLError as e:
        raise Exception('Could not get the fastcoll source code. Problem resolving the site.\nURL: {}\nError: {}'.format(FASTCOLL_LOC, e))
    data = resp.read()
    mf = io.BytesIO(data)
...
# attempt to compile fastcoll if the code is there but the binary does not exist
if not os.path.exists(os.path.join(FASTCOLL_PLACE, FASTCOLL_BIN)):
    back = os.getcwd()
...
def collide(ihv):
    ...
    plus = '  > /dev/null'
    r = os.system('./fastcoll --ihv {} -o {} {}{}'.format(ivhex, f0, f1, plus))
    if r != 0:
        raise Exception('Fastcoll binary failed. See stderr output above before this exception.')
